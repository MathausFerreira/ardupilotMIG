...
void Copter::ten_hz_logging_loop()
{
    ...
    if (should_log(MASK_LOG_NTUN) && (flightmode->requires_GPS() || !flightmode->has_manual_throttle())) {
...
void Copter::update_simple_mode(void)
{
    ...
    if (simple_mode == SimpleMode::SIMPLE) {
        ...
        y_force = channel_roll->get_control_in()*simple_cos_yaw - channel_pitch->get_control_in()*simple_sin_yaw;
        x_force = channel_roll->get_control_in()*simple_sin_yaw + channel_pitch->get_control_in()*simple_cos_yaw;
    }else{
        // rotate roll, pitch input by -super simple heading (reverse of heading to home)
        y_force = channel_roll->get_control_in()*super_simple_cos_yaw - channel_pitch->get_control_in()*super_simple_sin_yaw;
        x_force = channel_roll->get_control_in()*super_simple_sin_yaw + channel_pitch->get_control_in()*super_simple_cos_yaw;
    }

    // rotate roll, pitch input from north facing to vehicle's perspective
    channel_roll->set_control_in(y_force*ahrs.cos_yaw() + x_force*ahrs.sin_yaw());
    channel_pitch->set_control_in(-y_force*ahrs.sin_yaw() + x_force*ahrs.cos_yaw());
}
